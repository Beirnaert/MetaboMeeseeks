...
geom_cobweb <- function(mapping = NULL, data = NULL, stat = 'identity',
                           ...
                           show.legend = NA, inherit.aes = TRUE, ...) {
    ggplot2::layer(
        geom = geom_cobweb_proto, mapping = mapping,
        data = data, stat = stat, position = position, 
        show.legend = show.legend, inherit.aes = inherit.aes,
        params = list(na.rm = na.rm, ...)
...
required_aes = c("AUCs"),
default_aes = aes(fill = 0, colour = "black", alpha = 1, scale_radii = 1),
...
draw_group = function(data, panel_scales, coord) {
    ...
    coords <- coord$transform(data.df, panel_scales)
    #print(coords)
    
    # this you should do charlie
    # for(i in seq_along(unique(data$group))){
    #     
    #     df_points = base::data.frame(colour = data$colour,
    #                                  fill = data$fill,
    #                                  Polar2xy(AUCs = data$AUCs),
    #                                  group = data$group,
    #                                  PANEL = data$PANEL,
    #                                  alpha = data$alpha)
    # }
    
    # xyX <- Polar2xy(AUCs = data$AUCs)$x
    # xyY <- Polar2xy(AUCs = data$AUCs)$y
    # df_points = base::data.frame(colour = data$colour,
    #                              fill = data$fill,
    #                              x = xyX[1:(length(xyX)-1)],
    #                              y = xyY[1:(length(xyY)-1)],
    #                              group = data$group,
    #                              PANEL = data$PANEL,
    #                              alpha = data$alpha)
    df_points = base::data.frame(colour = data$colour,
...
geom_hurricane_proto <- ggplot2::ggproto("geom_hurricane_proto", Geom,
                                         ...
                                         draw_group = function(data, panel_scales, coord) {
                                             ...
                                             for (i in 1:nrow(data)) {
                                                 ...
                                                 df_points <- dplyr::bind_rows(list(df_nw))
                                             }
...
storm_observation <- load_hdata('/Users/Charlie/Downloads/ebtrk_atlc_1988_2015.txt') %>% 
    tidy_hdata() %>% 
...
ggplot(data = storm_observation,
       aes(x = longitude, y = latitude, 
           r_ne = ne, r_se = se, r_nw = nw, r_sw = sw,
           fill = wind_speed, color = wind_speed)) + geom_hurricane()



AUC.df = data.frame(AUCs = c(0.34, 0.52, 0.56, 0.4, 0.37, 0.45, rep(1/3,6)), type = c(rep("ROC",6), rep("Random", 6)))
#AUC.df = data.frame(AUCs = c(0.34, 0.52, 0.56, 0.4, rep(1/3,4)), type = c(rep("ROC",4), rep("Random", 4)))

ggplot(AUC.df, aes(AUCs = AUCs, group = as.factor(type), colour = as.numeric(as.factor(type)))) + geom_cobweb() + theme_bw()

ggplot(AUC.df, aes(AUCs = AUCs, group = as.factor(type), colour = as.numeric(as.factor(type)))) + coord_polar()+ geom_cobweb() + theme_bw()



ggplot() + geom_cobweb(AUC.df, aes(AUCs = AUCs)) + theme_bw()
